...
class QuestionListByCategoryView(generics.ListAPIView):
    serializer_class = QuestionSerializer
    permission_classes = [AllowAny]

    def get_queryset(self):
        category_name = self.kwargs['category'].lower()
...
- -100 <= Node.val <= 100''',
            'hints': 'You can solve this recursively or iteratively using a stack.'
        },
        {
            'category': arrays_category,
            'title': 'Merge Two Sorted Arrays',
            'question_text': '''You are given two integer arrays nums1 and nums2, sorted in non-decreasing order, and two integers m and n, representing the number of elements in nums1 and nums2 respectively.
...
- 0 <= m, n <= 200
- 1 <= m + n <= 200''',
            'hints': 'Start merging from the end of both arrays to avoid overwriting elements.'
        }
    ]
